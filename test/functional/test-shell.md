...
The `TestShell` submodule extends the `LearncoinTestFramework` functionality to
external interactive environments for prototyping and educational purposes. Just
like `LearncoinTestFramework`, the `TestShell` allows the user to:
...
to extend the object lifetime of the underlying `LearncoinTestFramework` between
...
## 2. Importing `TestShell` from the Learncoin Core repository

We can import the `TestShell` by adding the path of the Learncoin Core
...
The `TestShell` inherits all `LearncoinTestFramework` members and methods, such
as:
...
`LearncoinTestFramework` object. The full set of argument keywords which can be
...
Unlike the `LearncoinTestFramework` class, the `TestShell` keeps the underlying
Learncoind subprocesses (nodes) and logging utilities running until the user
explicitly shuts down the `TestShell` object.

During the time between the `setup` and `shutdown` calls, all `bitcoind` node
processes and `LearncoinTestFramework` convenience methods can be accessed
interactively.
...
underlying `LearncoinTestFramework`:
...
`LearncoinTestFramework` class. The user can override these in
