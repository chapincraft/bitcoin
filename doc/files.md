...
The data directory is the default location where the Learncoin Core files are stored.
...
---------|--------------------
Linux    | `$HOME/.bitcoin/`
macOS    | `$HOME/Library/Application Support/Learncoin/`
Windows  | `%APPDATA%\Learncoin\` <sup>[\[1\]](#note1)</sup>
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Learncoin blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
These subdirectories and files are no longer used by the Learncoin Core:
