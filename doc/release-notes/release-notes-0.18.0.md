Learncoin Core version 0.18.0 is now available from:
...
`/Applications/Learncoin-Qt` (on Mac) or `bitcoind`/`bitcoin-qt` (on
Linux).
...
Learncoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Learncoin Core on unsupported systems.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoincore.org/en/contact/) the Learncoin Core
...
`~bitcoin/.bitcoin`. This change makes Learncoin Core more consistent with
...
- A new document introduces Learncoin Core's BIP174 [Partially-Signed
  Learncoin Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Learncoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Learncoin Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Learncoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Learncoin Core, subsequently importing
...
- A new `bitcoin-wallet` tool is now distributed alongside Learncoin
...
This section describes planned changes to Learncoin Core that may affect
other Learncoin software and services.

- Since version 0.16.0, Learncoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Learncoin Core 0.20 (expected about
  a year after 0.18), Learncoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Learncoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Learncoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Learncoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Learncoin Core
...
- A sub-project of Learncoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Learncoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Learncoin Core's own implementation, although entropy
  gathered by Learncoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Learncoin Core a
...
- On macOS, Learncoin Core now opts out of application CPU throttling
...
- #11625 Add LearncoinApplication & RPCConsole tests (ryanofsky)
...
- #11526 Visual Studio build configuration for Learncoin Core (sipsorcery)
...
- #12775 Integration of property based testing into Learncoin Core (Christewart)
